...
plot1 <- ggplot(filtered_factored_mobileTrain, aes(price_range)) +
  geom_bar(aes(fill = blue)) +
  ggtitle("Price Range with Bluetooth Overlay") +
  theme(legend.text = element_text(size = 8),    # Adjust the legend text 
        axis.text.y = element_text(size = 8),    # Adjust the y-axis text 
        axis.text.x = element_text(size = 8),    # Adjust the x-axis text 
        plot.title = element_text(size = 10))    # Adjust the title text 

plot2 <- ggplot(filtered_factored_mobileTrain, aes(price_range)) +
  geom_bar(aes(fill = dual_sim)) +
  ggtitle("Price Range with Dual Sim Overlay") +
  theme(legend.text = element_text(size = 8),      
        axis.text.y = element_text(size = 8),      
        axis.text.x = element_text(size = 8),      
        plot.title = element_text(size = 10))      

plot3 <- ggplot(filtered_factored_mobileTrain, aes(price_range)) +
  geom_bar(aes(fill = four_g)) +
  ggtitle("Price Range with 4G Overlay") +
  theme(legend.text = element_text(size = 8),      
        axis.text.y = element_text(size = 8),      
        axis.text.x = element_text(size = 8),      
        plot.title = element_text(size = 10))      

plot4 <- ggplot(filtered_factored_mobileTrain, aes(price_range)) +
  geom_bar(aes(fill = three_g)) +
  ggtitle("Price Range with 3G Overlay") +
  theme(legend.text = element_text(size = 8),      
        axis.text.y = element_text(size = 8),      
        axis.text.x = element_text(size = 8),      
        plot.title = element_text(size = 10))      

plot5 <- ggplot(filtered_factored_mobileTrain, aes(price_range)) +
  geom_bar(aes(fill = touch_screen)) +
  ggtitle("Price Range with Touch Screen Overlay") +
  theme(legend.text = element_text(size = 8),      
        axis.text.y = element_text(size = 8),      
        axis.text.x = element_text(size = 8),      
        plot.title = element_text(size = 10))      

plot6 <- ggplot(filtered_factored_mobileTrain, aes(price_range)) +
  geom_bar(aes(fill = wifi)) +
  ggtitle("Price Range with Wifi Overlay") +
  theme(legend.text = element_text(size = 8),      
        axis.text.y = element_text(size = 8),      
        axis.text.x = element_text(size = 8),      
        plot.title = element_text(size = 10))      

# Arrange plots in a 3x2 matrix
grid.arrange(
...
for (i in levels(MobileTesting$price_range)) {
  ...
  # Calculate AUC
  roc_obj <- roc(response_C50, predictor_C50, levels = c(0, 1))
  auc_C50_list[[i]] <- auc(roc_obj)
}

# Calculate average AUC
auc_C50 <- round(mean(unlist(auc_C50_list)),4)
...
for (i in levels(fold_test$price_range)) {
  # Create vectors of the same length
  response <- as.numeric(MobileTestingKNN$price_range == i)
  predictor <- as.numeric(ypred_knn)[1:nrow(MobileTestingKNN)]
...
for (i in levels(MobileTestingSVM$price_range)) {
  # Create vectors of the same length
  response_LR <- as.numeric(MobileTestingLR$price_range == i)
  predictor_LR <- as.numeric(ypred_logreg)[1:nrow(MobileTestingLR)]

  # Calculate AUC
  roc_obj_LR <- roc(response_LR, predictor_LR, levels = c(0, 1))
  auc_list_LR[[i]] <- auc(roc_obj_LR)
}

# Calculate average AUC
auc_LR <- round(mean(unlist(auc_list_LR)),4)

# Print AUC
cat("Multi-class AUC for Logistic Regression:", auc_LR, "\n")
})
```
...
for (i in levels(MobileTestingSVM$price_range)) {
  # Create vectors of the same length
  response_NB <- as.numeric(MobileTestingNB$price_range == i)
  predictor_NB <- as.numeric(ypred_nb)[1:nrow(MobileTestingNB)]

  # Calculate AUC
  roc_obj_NB <- roc(response_NB, predictor_NB, levels = c(0, 1))
  auc_list_NB[[i]] <- auc(roc_obj_NB)
}

# Calculate average AUC
auc_NB <- round(mean(unlist(auc_list_NB)),4)

# Print AUC
cat("Multi-class AUC for Naive Bayes:", auc_NB, "\n")
})
```
